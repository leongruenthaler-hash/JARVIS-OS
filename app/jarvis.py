...
def run_set_openai_key() -> int:
    try:
        ...
        removed = remove_openai_key_from_env_file(Path(__file__).resolve().parent.parent)
        print(message)
        if removed:
...
def run_privacy_test() -> int:
    ...
    try:
        for path in sorted((Path(__file__).resolve().parent.parent / "memory").glob("*.json")) + sorted(Path(__file__).resolve().parent.glob("*.json")):
    ...
    try:
        ...
        config_text = (root / "app" / "config.json").read_text(encoding="utf-8", errors="ignore")
