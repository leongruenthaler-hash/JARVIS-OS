...
class ModelManager:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        try:
            ...
            has_key = False
        if data.get("provider") == "openai" and (not bool(data.get("openai_enabled")) or not has_key):
            data["provider"] = "ollama"
            changed = True
        if not has_key and data.get("openai_enabled") is not False:
    ...
    def use_claude_code(self) -> str:
        if not is_claude_code_available():
            self.data["provider"] = "ollama"
            self._save()
            return (
                "Claude Code ist nicht verfuegbar. Installiere die 'claude'-CLI und melde dich "
                "einmalig per 'claude login' an."
            )
        self.data["provider"] = "claude_code"
        self.data["openai_enabled"] = False
        self._save()
    ...
    def work_locally(self) -> str:
        ...
        self.data["openai_enabled"] = False
        self._save()
        return f"Lokaler Modus aktiv. Cloud-Anbieter sind deaktiviert. Ich nutze {self.data.get('local_model', DEFAULT_LOCAL_MODEL)}."
